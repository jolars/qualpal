<script lang="ts">
  ...
  export let onChange: (values: { hueMin: number; hueMax: number }) => void = () => {};
...
<svg width="200" height="200" style="touch-action:none;">
  ...
  <foreignObject x="0" y="0" width="200" height="200">
    <div style="
      ...
      mask: radial-gradient(transparent {radius - 8}px, black {radius - 8}px, black {radius + 8}px, transparent {radius + 8}px);
      -webkit-mask: radial-gradient(transparent {radius - 8}px, black {radius - 8}px, black {radius + 8}px, transparent {radius + 8}px);
    "></div>
  </foreignObject>
  <circle cx={center} cy={center} r={radius - 8} fill="none" stroke="#f3f4f6" stroke-width="2"/>
  <circle cx={center} cy={center} r={radius + 8} fill="none" stroke="#f3f4f6" stroke-width="2"/>
  <path
    ...
    stroke="rgba(0,0,0,0.7)" stroke-width="16" fill="none"
  />
  <!-- Handles -->
  <circle
