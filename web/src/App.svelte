<script>
  ...
  $: colorOutput = JSON.stringify(palette.map(color => color.hex), null, 2);
...
<div class="min-h-screen bg-gray-100">
  ...
  <header class="bg-white shadow-sm border-b">
    <div class="max-w-7xl mx-auto px-4 py-4">
      ...
      <p class="text-gray-600 mt-1">Generate perceptually uniform color palettes for data visualization</p>
    </div>
  </header>
          ...
          <div class="space-y-6">
            ...
            <div class="form-group">
              <label class="block text-sm font-medium text-gray-700 mb-2">
                Number of Colors
                <span class="text-blue-600 font-semibold">{params.numColors}</span>
              </label>
              <input 
                type="range" 
                min="2" 
                ...
                <div class="group">
                  ...
                  >
                    <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-10 transition-all duration-200 flex items-center justify-center">
                      <span class="text-white opacity-0 group-hover:opacity-100 font-medium text-sm bg-black bg-opacity-50 px-2 py-1 rounded">
                        Copy
                      </span>
                    </div>
                  </div>
                  <div class="mt-2 text-center">
                    <div class="text-sm font-mono text-gray-700">{color.hex}</div>
                  </div>
                </div>
              {/each}
            </div>
...
<div class="bg-white rounded-lg shadow-sm border p-6">
  ...
  <div class="mb-4 border-b flex gap-2">
    {#each ['JSON', 'R', 'Python', 'CSS'] as tab}
      <button
        class="px-4 py-2 -mb-px border-b-2 font-mono text-sm transition-colors
          {activeTab === tab
  ...
  <div class="relative bg-gray-50 rounded-lg p-4 border mt-2">
    ...
    <pre class="text-sm font-mono text-gray-700 overflow-x-auto">{outputText}</pre>
  </div>
</div>
        </div>
      {:else if moduleLoaded}
        <div class="bg-white rounded-lg shadow-sm border p-12 text-center">
          ...
          <h2 class="text-xl font-semibold text-gray-900 mb-2">Ready to Generate</h2>
          <p class="text-gray-600">Adjust the parameters in the sidebar and click "Generate New Palette" to get started.</p>
        </div>
      {/if}
    </main>
  </div>
  ...
  <!-- Toast Notification -->
  {#if toast.show}
    <div class="fixed bottom-4 right-4 bg-gray-800 text-white px-4 py-3 rounded-lg shadow-lg z-50 transition-all duration-300">
